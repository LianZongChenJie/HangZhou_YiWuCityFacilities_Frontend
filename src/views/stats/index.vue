    ...
    <div class="search-card">
      <el-form :inline="true" :model="q">
        ...
        <el-form-item class="search-actions">
          ...
          <el-button type="success" :loading="exportLoading" @click="exportXls">导出 Excel</el-button>
        </el-form-item>
      </el-form>
    </div>
...
function buildParams(): Request {
  const params: Request = {
    pageNo: q.pageNo,
    pageSize: q.pageSize,
    acceptDates: q.acceptDates?.length === 2 ? q.acceptDates : undefined,
  ...
  // 金额区间
  const amount: string[] = []
  if (q.minAmt !== '') amount.push(q.minAmt)
  if (q.maxAmt !== '') amount.push(q.maxAmt)
...
async function fetchData() {
  ...
  try {
    ...
    const s = summaryRes || {}
    summary.value = [
      { label: '办件数量', value: s.total ?? s.count ?? 0 },
      { label: '应收合计（元）', value: formatMoney(s.receivableTotal ?? s.receivable ?? 0) },
      { label: '已到账合计（元）', value: formatMoney(s.paidTotal ?? s.paid ?? 0) },
      { label: '待缴款数量', value: s.pendingCount ?? s.pending ?? 0 },
      { label: '免征办件数量', value: s.reductionCount ?? s.reductionCount ?? 0 },
      { label: '免征金额合计（元）', value: formatMoney(s.reductionAmountTotal ?? s.reductionAmount ?? 0) }
    ]
  } catch (e: any) {
